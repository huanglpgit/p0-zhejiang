            ...
            <div class="row redisEcharts-box">
              ...
              <div class="echarts-legend-own echarts-legend-alarm" v-show="!noData">
                <ul>
                  ...
                  <li class="cursor J_legendStatic" v-show="J_legendStaticShow" :class="{'off':J_legendStaticOffClass}" @click="legendStaticClk">
  ...
  data() {
      ...
      //有日期控件时import之后需要再增加methods
			...
			tipFormatter(params) {
				...
				var curVal = (params[0].value[1] == "" && params[1].value[1] == "") || (params[0].value[1] == "" && params[1].value[1] == "NaN") || (params[0].value[1] == "" && params[1].value[1] == "00") ? "--" : accAdd(params[0].value[1], params[1].value[1]);
				//要求tip提示顺序是时间-预测值2-上限1-下限0-实际值3
				if(params && params.length > 0) {
					var val2 = '',
						val3 = '';
					var val0 = params[0].value[1] == '' || params[0].value[1] == null || params[0].value[1] == undefined ? "--" : params[0].value[1] + chartsUnit;
					if(params[2]) {
						val2 = params[2].value[1] == '' || params[2].value[1] == null || params[2].value[1] == undefined ? "--" : params[2].value[1] + chartsUnit;
					} else {
						val2 = "--"
					}
					if(params[3]) {
						if(params[3].value[1] == '' || params[3].value[1] == null || params[3].value[1] == undefined) {
							val3 = '--'
						} else {
					...
					tooltipStr = '<div class="tip of-hidden"><p><span class="tip-left">时间:</span><span class="tip-right">' + time + '</span></p></div>' +
						'<div class="tip of-hidden"><p><span class="tip-left">预测值:</span><span class="tip-right">' + val2 + '</span></p></div>' +
						'<div class="tip of-hidden"><p><span class="tip-left">上限:</span><span class="tip-right">' + valShangxian + '</span></p></div>' +
						'<div class="tip of-hidden"><p><span class="tip-left">下限:</span><span class="tip-right">' + val0 + '</span></p></div>' +
						'<div class="tip of-hidden"><p><span class="tip-left">实际值:</span><span class="tip-right J_shiji">' + val3 + '</span></p></div>' +
						'<div class="tip of-hidden"><p><span class="tip-left">级别:</span><span class="tip-right J_shiji">' + valLevel + '</span></p></div>'
					return tooltipStr;
				}
			},
			//获取列表
			async getCharts() {
			  ...
			  that.loadingGi = true;
				...
				if(that.searchs.trainIndex == "ha_ins_resp_time" && that.searchs.ip == "") {
        ...
        var  result = await getPageDisplay(requestData);
         that.loadingGi = false;
					...
					if(result.status == 200) {
							...
							//小标题
							let zhuanyeCur = that.zhuanyeList.filter(item => item.type==that.searchs.zhuanye ),//指标类型
								trainIndexCur = that.trainIndexList.filter(item => item.trainType==that.searchs.trainIndex ),//预测指标
								type_name = zhuanyeCur.length>0 ? zhuanyeCur[0].type_name : '指标类型',
							    trainTypeName = trainIndexCur.length>0 ? trainIndexCur[0].trainTypeName : '预测指标';
							that.tableCellName = type_name +'-'+trainTypeName+'-'+(that.searchs.ip ||'')
							

							
							var data = result.data.series; //所有值集合 顺序 0预测值-1实际值-2上限值-3下限值-4预测值
							var xaxis = result.data.xaxis && result.data.xaxis.data ? result.data.xaxis.data : []; //横坐标
							var numObj = result.data; //返回的数据
							
							
							if(xaxis.length <= 0) {
								that.myChart = that.$echarts.init(document.getElementById('chartsConnects'));
								that.myChart.clear();
								that.noData = true;
								return;
							} else {
              ...
              } else if(numObj.isWork == "N") { //控制静态阈值图例的样式
               ...
               that.J_legendStaticOffClass = true;
                ...
                that.J_legendStaticShow = true;
							...
							var dataMinArr = [],dataMaxArr = [],dataVirtualArr = [],dataActualArr = [],dataStaticMinArr = [],dataStaticMaxArr = []; 
							//真正开始的地方
							...
							for(var p = 0; p < dataActualNotnullsX.length - 1; p++) {
								...
								var result = new Date(datetimeNow2).getTime() - new Date(datetimeNow1).getTime();
								//这个要动态获取训练配置的时间粒度（分钟），然后再*2，>时间粒度*2的就不连 by秋焱20190926
								
								if(result > 2*granularity*60 * 1000) {
							...
							var startValue = 0,xTimeLast = xaxis[xaxis.length-1],startTime = 0;
							//20191209 暂时注释
							...
							if(that.searchs.dataType == "W") { //展示近一天 
								startTime = new Date(xTimeLast).getTime()-24*60*60*1000;
								//startValue = DateFmt.Formate(startTime);
								startValue = moment(startTime).format("YYYY-MM-DD HH:mm:ss");
							}
							if(that.searchs.dataType == "M") { //展示近两天
							...
							for(var i = 0; i < xaxis.length; i++) {
								var b = dataMax[i] == "" && dataMin[i] == "" ? "00" : accSub(dataMax[i], dataMin[i]);
							...
							var option = {
								...
								legend: {
									...
									data: ["下限", "上限", "预测值", "实际值", "静态阈值上", "静态阈值下"],
									//实现初始某一项默认不选择
									selected: {　　　　
								...
								visualMap: { //是否连接空数据 20190802 巧妙解决折线断裂连接 然后真正断裂的地方使用透明色假性过滤掉（呈现数据不连续的断裂效果）
								...
								series: [{
									...
									},
									{
										...
										symbol:'react',
										symbolSize:0.1,
										itemStyle: {//折点颜色自定义 解决无连线是折点看不到 误以为此处无值
											normal: {
												borderColor: '#F6AD34',
												borderWidth:1
											}
      ...
      async getListZhuanye() { 
        ...
        if(res.status=="200"){
            ...
            if(this.trainIndexList.length>=0){
              this.unit = this.trainIndexList[0].unit ? this.trainIndexList[0].unit : '';
              this.granularity = this.trainIndexList[0].granularity ? this.trainIndexList[0].granularity : '';
