          ...
          <a-col :span="20">
            <a-row type="flex" justify="start">
              ...
              <a-col>
                ...
                >
                  <template slot="dataSource">
                    <a-select-option
                      v-for="(item,index) in alarmObjTypeList"
                      :key="'alarmObjType'+index"
                      :value="item.alarmObjType"
                    >{{item.alarmObjType}}</a-select-option>
                  </template>
                </a-auto-complete>
              </a-col>
              <a-col>
              ...
              <a-col>
                ...
                <a-input
                  placeholder="告警标题/告警对象名称/告警描述"
                  v-model.trim="searchs.text"
                  style="width:260px;"
                />
              </a-col>
              <a-col>
    ...
    <div v-show="showSecPage" class="sec-wrp-outer gi-outer-wrp" id="secondWrp">
      ...
      <div class="sec-table-wrp">
        ...
        >
          <!--tooltip 提示-->
          <span
            v-for="(item,index) in columnsSec"
            :key="index"
            :slot="item.dataIndex"
            slot-scope="text"
          >
          ...
          <span slot="rcAlarmType" slot-scope="rcAlarmType">
            <span v-if="rcAlarmType=='R'">根因告警</span>
            <span v-if="rcAlarmType!='R'">{{rcAlarmType}}</span>
          </span>
        </a-table>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getListMasterType(value) {
      var res = await getMasterTypesCause({
        domain: this.searchs.domain,
        alarmObjType: value
      });
